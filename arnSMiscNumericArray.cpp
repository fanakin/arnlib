/***************************************************************************
                          arnSMiscNumericArray.cpp  -  description
                             -------------------
...
// ***************************************************
// ---------------------------------------------------
arnSMiscNumericArray::arnSMiscNumericArray ( QObject * parent, const char * name )
...
arnSMiscNumericArray::~arnSMiscNumericArray(void)
{
//arnDebug("%s\n",text(0).latin1());
}

void arnSMiscNumericArray::init(void)
{
    tpe = sot_MiscNumericArray;
    xAxis = 0;
    NRows = 5;
    prcX = 0;
    mnX = 0;
...
void arnSMiscNumericArray::setValue(int row, double val)
{
    ...
    switch (elemformat) {
        ...
        case nff_s16:
            switch (filterT) {
                case np_linear:
					(*((signed short*) &(pDataFrame[offst + (2 * row)]))) = (signed short) smirror((unsigned short) arnNumericFilter::toRawSignedShort(val,mn,mx));
                    break;
                case np_none:
                default:
        ...
        case nff_s32:
            switch (filterT) {
                case np_linear:
					(*((signed long*) &(pDataFrame[offst + (4 * row)]))) = (signed long) lmirror((unsigned long) arnNumericFilter::toRawSignedLong(val,mn,mx));
                    break;
                case np_none:
                default:
...
void arnSMiscNumericArray::setValue(int row, const QString& val)
{
    bool ok;
...
void arnSMiscNumericArray::setValue(int row, int col)
{
...
double arnSMiscNumericArray::valueX(int row)
{
	if (!xAxis) {
...
double arnSMiscNumericArray::value(int row)
{
    if (!pDataFrame) arnCriticalExit();
    if (elemformat == nff_none) arnCriticalExit();
    if (!sze || sze < ((unsigned long) (typeTonbytes() * row))) arnCriticalExit();
    switch (elemformat) {
        ...
        case nff_u32:
            switch (filterT) {
                case np_linear:
                    return  arnNumericFilter::linear(lmirror((*((unsigned long*) &(pDataFrame[offst + (4 * row)])))),mn,mx);
                case np_none:
                default:
        ...
        case nff_s16:
            switch (filterT) {
                case np_linear:
                    return  arnNumericFilter::linear((signed short) smirror((*((signed short*) &(pDataFrame[offst + (2 * row)])))),mn,mx);
                case np_none:
                default:
...
double arnSMiscNumericArray::rawvalue(int row)
{
	if (!pDataFrame) arnCriticalExit();
	if (elemformat == nff_none) arnCriticalExit();
	if (!sze || sze < ((unsigned long) (typeTonbytes() * row))) arnCriticalExit();
