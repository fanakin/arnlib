/***************************************************************************
                          arnMiscMatrixViewer.cpp  -  description
                             -------------------
...
#include "arnMiscMatrixViewer.h"
#include <QContextMenuEvent>

arnMiscMatrixViewer::arnMiscMatrixViewer(int rows, int cols, QWidget *parent, const char *name, Qt::WFlags f)
    : arnObjectViewer(parent, name, f)
{
    tpy = avt_MiscMatrixViewer;
	...
	mnCol = new double[pMatrix->numCols()];
	if (!mnCol) arnCriticalExit();
	mxCol = new double[pMatrix->numCols()];
	if (!mxCol) arnCriticalExit();
	prcCol = new signed char[pMatrix->numCols()];
...
//    SelezionaTutto->addTo(pPopUp);
    ...
    prec = precY = -1;
	for (int indx = 0; indx < pMatrix->numCols(); indx++) {
		mnCol[indx] = 0;
		mxCol[indx] = pMatrix->numCols();
		prcCol[indx] = precY;
		}
...
arnMiscMatrixViewer::arnMiscMatrixViewer(QWidget *parent, const char *name, Qt::WFlags f)
    : arnObjectViewer(parent, name, f)
{
    tpy = avt_MiscMatrixViewer;
    pMainLayout = new QVBoxLayout(this, 10, -1, "MainLayout");
...
arnMiscMatrixViewer::~arnMiscMatrixViewer(void)
{
...
void arnMiscMatrixViewer::setXValue(int col, const QString& val)
{
    pMatrix->horizontalHeader()->setLabel(col,val);
}

void arnMiscMatrixViewer::setYValue(int row, double val)
{
	if (precY == -1) {
		switch (numFormatY) {
			case nrf_hex:
				if (pMatrix) pMatrix->horizontalHeader()->setLabel(row,QString::number((int)val, 16).toUpper());
			break;
			case nrf_bin:
				if (pMatrix) pMatrix->horizontalHeader()->setLabel(row,QString::number((int)val, 2));
			break;
			case nrf_dec:
			default:
...
void arnMiscMatrixViewer::setValue(int row, int col, double val)
{
    QString ts;
    if (PrecCol(col) == -1) pMatrix->setText(row,col,ts.setNum(val));
    else  pMatrix->setText(row,col,ts.setNum(val,'f',PrecCol(col)));
}

QString arnMiscMatrixViewer::text(int row, int col)
{
    QString ts;
...
void arnMiscMatrixViewer::contextMenuEvent ( QContextMenuEvent * e )
{
//arnDebug("Entrato\n");
...
// bool arnMiscMatrixViewer::eventFilter(QObject* watched, QEvent* e)
// {
...
void arnMiscMatrixViewer::Inizializza_Hndl(void)
{
    bool ok = false;
...
void arnMiscMatrixViewer::Interpola_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
...
void arnMiscMatrixViewer::InterpolaX_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
        for (int indR = sel.topRow(); indR <= sel.bottomRow(); indR++) {
...
void arnMiscMatrixViewer::InterpolaY_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
        for (int indC = sel.leftCol() ; indC <= sel.rightCol(); indC++) {
...
void arnMiscMatrixViewer::Azzera_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
        for (int indR = sel.topRow(); indR <= sel.bottomRow(); indR++) {
...
void arnMiscMatrixViewer::Aggiungi_Hndl(void)
{
    bool ok = false;
    double res = QInputDialog::getDouble(
        tr( "Aggiungi" ),
...
void arnMiscMatrixViewer::Moltiplica_Hndl(void)
{
    bool ok = false;
    double res = QInputDialog::getDouble(
        tr( "Aggiungi" ),
...
void arnMiscMatrixViewer::Nascondi_Hndl(void)
{
...
void arnMiscMatrixViewer::Mostratutti_Hndl(void)
{
...
void arnMiscMatrixViewer::Copia_Hndl(void)
{
	QString aTxt("MATRIX\n");
	int minR, minC;
...
void arnMiscMatrixViewer::Incolla_Hndl(void)
{
	if (pClip) {
		if (pClip->supportsSelection()) {
...
void arnMiscMatrixViewer::selectCells(int sRow, int sCol, int eRow, int eCol)
{
...
void arnMiscMatrixViewer::doValueChange(int row, int col)
{
    emit valueChanged(row,col);
}

void arnMiscMatrixViewer::enterEvent(QEvent */*e*/)
{
	if (pClip) {
		if (pClip->supportsSelection()) {
