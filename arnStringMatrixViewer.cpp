/***************************************************************************
						  arnStringMatrixViewer.cpp  -  description
                             -------------------
...
#include "arnStringMatrixViewer.h"
#include <QContextMenuEvent>

arnStringMatrixViewer::arnStringMatrixViewer(int rows, int cols, QWidget *parent, const char *name, Qt::WFlags f)
    : arnObjectViewer(parent, name, f)
{
	tpy = avt_StringMatrixViewer;
	numFormatX = nrf_dec;
	numFormatY = nrf_dec;
...
arnStringMatrixViewer::arnStringMatrixViewer(QWidget *parent, const char *name, Qt::WFlags f)
    : arnObjectViewer(parent, name, f)
{
...
arnStringMatrixViewer::~arnStringMatrixViewer(void)
{

}

void arnStringMatrixViewer::setXValue(int col, double val)
{
	if (!precX) {
		switch (numFormatX) {
			case nrf_hex:
...
void arnStringMatrixViewer::setYValue(int row, double val)
{
	if (!precY) {
		switch (numFormatY) {
			case nrf_hex:
...
void arnStringMatrixViewer::setValue(int row, int col, const QString& val)
{
	QString ts = val;
	if (!prec) pMatrix->setText(row,col,ts);
	else  pMatrix->setText(row,col,ts);
}

QString arnStringMatrixViewer::text(int row, int col)
{
    QString ts;
...
void arnStringMatrixViewer::contextMenuEvent ( QContextMenuEvent * e )
{
//arnDebug("Entrato\n");
...
// bool arnStringMatrixViewer::eventFilter(QObject* watched, QEvent* e)
// {
...
//void arnStringMatrixViewer::SelezionaTutto_Hndl(void)
//{
//    pMatrix->clearSelection();
//}

void arnStringMatrixViewer::Inizializza_Hndl(void)
{
    bool ok = false;
...
void arnStringMatrixViewer::Interpola_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
...
void arnStringMatrixViewer::InterpolaX_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
        for (int indR = sel.topRow(); indR <= sel.bottomRow(); indR++) {
...
void arnStringMatrixViewer::InterpolaY_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
        for (int indC = sel.leftCol() ; indC <= sel.rightCol(); indC++) {
...
void arnStringMatrixViewer::Azzera_Hndl(void)
{
    for (int indxSel = 0; indxSel < pMatrix->numSelections(); indxSel++) {
        Q3TableSelection sel = pMatrix->selection(indxSel);
        for (int indR = sel.topRow(); indR <= sel.bottomRow(); indR++) {
...
void arnStringMatrixViewer::Aggiungi_Hndl(void)
{
    bool ok = false;
...
void arnStringMatrixViewer::Moltiplica_Hndl(void)
{
    bool ok = false;
    double res = QInputDialog::getDouble(
        tr( "Aggiungi" ),
...
void arnStringMatrixViewer::Nascondi_Hndl(void)
{
...
void arnStringMatrixViewer::Mostratutti_Hndl(void)
{
...
void arnStringMatrixViewer::Copia_Hndl(void)
{
	QString aTxt("MATRIX\n");
	int minR, minC;
...
void arnStringMatrixViewer::Incolla_Hndl(void)
{
	if (pClip) {
		if (pClip->supportsSelection()) {
...
void arnStringMatrixViewer::selectCells(int sRow, int sCol, int eRow, int eCol)
{
...
void arnStringMatrixViewer::doValueChange(int row, int col)
{
    emit valueChanged(row,col);
}

void arnStringMatrixViewer::enterEvent(QEvent *)
{
	if (pClip) {
		if (pClip->supportsSelection()) {
