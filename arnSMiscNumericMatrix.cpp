/***************************************************************************
                          arnSMiscNumericMatrix.cpp  -  description
                             -------------------
...
// ***************************************************
// ---------------------------------------------------
arnSMiscNumericMatrix::arnSMiscNumericMatrix ( QObject * parent, const char * name )
    : arnSMatrix( parent, name)
{
    this->init();
}

arnSMiscNumericMatrix::~arnSMiscNumericMatrix(void)
{
//arnDebug("%s\n",text(0).latin1());
}

void arnSMiscNumericMatrix::init(void)
{
    tpe = sot_MiscNumericMatrix;
    xAxis = 0;
    yAxis = 0;
    prcX = 0;
    mnX = 0;
...
void arnSMiscNumericMatrix::setDims(int rows, int cols)
{
    if (!xAxis) NCols = cols;
...
void arnSMiscNumericMatrix::setValue(int row, int col, double val)
{
    ...
    switch (elemformat) {
        ...
        case nff_s16:
            switch (filterT) {
                case np_linear:
                    (*((signed short*) &(pDataFrame[offst + (2 * ((row * NCols) + col))]))) = smirror((signed short)arnNumericFilter::toRawSignedShort(val,mn,mx));
                    break;
                case np_none:
                default:
...
void arnSMiscNumericMatrix::setValue(int row, int col, const QString& val)
{
    bool ok;
...
void arnSMiscNumericMatrix::setValue(int row, int col)
{
...
double arnSMiscNumericMatrix::valueX(int cols)
{
	if (!xAxis) {
...
double arnSMiscNumericMatrix::valueY(int rows)
{
	if (!yAxis) {
...
double arnSMiscNumericMatrix::value(int row, int col)
{
    if (!pDataFrame) arnCriticalExit();
    if (elemformat == nff_none) arnCriticalExit();
    if (!sze || sze < ((unsigned long) (typeTonbytes() * ((row * NCols) + col)))) arnCriticalExit();
    switch (elemformat) {
        ...
        case nff_u32:
            switch (filterT) {
                case np_linear:
                    return  arnNumericFilter::linear((lmirror((*((unsigned long*) &(pDataFrame[offst + (4 * ((row * NCols) + col))]))))),mn,mx);
                case np_none:
                default:
        ...
        case nff_s16:
            switch (filterT) {
                case np_linear:
                    return  arnNumericFilter::linear((signed short)(smirror((*((unsigned short*) &(pDataFrame[offst + (2 * ((row * NCols) + col))]))))),mn,mx);
                case np_none:
                default:
        ...
        case nff_s32:
            switch (filterT) {
                case np_linear:
                    return  arnNumericFilter::linear((signed long)(lmirror((*((unsigned long*) &(pDataFrame[offst + (4 * ((row * NCols) + col))]))))),mn,mx);
                case np_none:
                default:
...
double arnSMiscNumericMatrix::rawvalue(int row, int col)
{
	if (!pDataFrame) arnCriticalExit();
	if (elemformat == nff_none) arnCriticalExit();
	if (!sze || sze < ((unsigned long) (typeTonbytes() * ((row * NCols) + col)))) arnCriticalExit();
