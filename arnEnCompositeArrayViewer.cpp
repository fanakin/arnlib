/***************************************************************************
                          arnEnCompositeArrayViewer.cpp  -  description
                             -------------------
...
void arnEnCompositeArrayViewer::init(int rows)
{
    tpy = avt_EnCompositeArrayViewer;
	...
	for (int i = 0; i < pArray->numRows(); i++) {
		arnColorTableItem* pIt = new arnColorTableItem(pArray,Q3TableItem:: OnTyping);
		pArray->setItem(i,0,pIt);
		}
    ...
    pComment = new QLabel( this, "pComment" );
    pComment->setMinimumSize( QSize( 300, 0 ) );
...
arnEnCompositeArrayViewer::arnEnCompositeArrayViewer(int rows, QWidget *parent, const char *name, Qt::WFlags f)
    : arnObjectViewer(parent, name, f)
{
	init(rows);
}

arnEnCompositeArrayViewer::arnEnCompositeArrayViewer(QWidget *parent, const char *name, Qt::WFlags f)
    : arnObjectViewer(parent, name, f)
{
	init(5);
}

arnEnCompositeArrayViewer::~arnEnCompositeArrayViewer(void)
{
	for (int i = 0; i < pArray->numRows(); i++) {
		arnColorTableItem* pIt = (arnColorTableItem *)(pArray->item( i, 0 ));
		if (pIt)  {delete pIt; pIt = 0;}
		}
}

void arnEnCompositeArrayViewer::setXValue(int row, double val)
{
    QString ts;
	if (row < pArray->numRows()) {
		if (!precX) {
			pArray->verticalHeader()->setLabel(row,ts.setNum(val));
			if (pGLArray) pGLArray->setXValue(row,val);
			}
		else {
			pArray->verticalHeader()->setLabel(row,ts.setNum(val,'f',precX));
			if (pGLArray) pGLArray->setXValue(row,val);
			}
		}
}

void arnEnCompositeArrayViewer::setValue(int row, double val)
{
	if (row < pArray->numRows()) {
		QString ts;
		if (!prec) {
...
QString arnEnCompositeArrayViewer::text(int row)
{
    QString ts("");
...
QString arnEnCompositeArrayViewer::text(int row,int col)
{
    QString ts("");
...
void arnEnCompositeArrayViewer::contextMenuEvent ( QContextMenuEvent * e )
{
//arnDebug("Entrato\n");
...
// bool arnEnCompositeArrayViewer::eventFilter(QObject* watched, QEvent* e)
// {
...
void arnEnCompositeArrayViewer::Inizializza_Hndl(void)
{
    bool ok = false;
...
void arnEnCompositeArrayViewer::Interpola_Hndl(void)
{
    if (pArray->isColumnSelected(pArray->currentColumn(),true)) {
...
void arnEnCompositeArrayViewer::Azzera_Hndl(void)
{
    if (pArray->isColumnSelected(pArray->currentColumn(),true)) {
            for (int indx = 0; indx < pArray->numRows(); indx++) {
            setValue(indx,0);
...
void arnEnCompositeArrayViewer::Aggiungi_Hndl(void)
{
    bool ok = false;
    double res = QInputDialog::getDouble(
        tr( "Aggiungi" ),
...
void arnEnCompositeArrayViewer::Moltiplica_Hndl(void)
{
    bool ok = false;
    double res = QInputDialog::getDouble(
        tr( "Aggiungi" ),
...
void arnEnCompositeArrayViewer::Nascondi_Hndl(void)
{
    if (pArray->isColumnSelected(pArray->currentColumn(),true)) {
        for (int indx = 0; indx < pArray->numRows(); indx++) {
            pArray->hideRow(indx);
...
void arnEnCompositeArrayViewer::Mostratutti_Hndl(void)
{
    for (int indx = 0; indx < pArray->numRows(); indx++) {
        pArray->showRow(indx);
        }
...
void arnEnCompositeArrayViewer::Copia_Hndl(void)
{
		QString aTxt("VECTOR\n");
		int minR, minC;
		minR = pArray->numRows();
...
void arnEnCompositeArrayViewer::Incolla_Hndl(void)
{
	if (pClip) {
		if (pClip->supportsSelection()) {
...
void arnEnCompositeArrayViewer::selectCells(int sRow, int eRow)
{
//	Q3TableSelection sel;
//	sel.init(sRow,1);
//	sel.expandTo(eRow,1);
...
void arnEnCompositeArrayViewer::doValueChange(int row, int col)
{
	if (pGLArray) {
...
void arnEnCompositeArrayViewer::languageChange()
{
    LXValLabel->setText( tr( "X" ) );
...
void arnEnCompositeArrayViewer::ArrayViewerButton_hndl(void)
{
...
void arnEnCompositeArrayViewer::Graph2DViewerButton_hndl(void)
{
	QList<int> sList = mainsplitter->sizes();
	for (int i = 0; i < sList.size() - 1; ++i) {
		if (sList.at(i)) {
...
void arnEnCompositeArrayViewer::Graph2DSelection_hndl(int start_row, int end_row)
{
	disconnect(YVallineEdit, SIGNAL(textChanged(const QString&)),this , SLOT(YLineEditTextChange_hndl(const QString&)));
...
void arnEnCompositeArrayViewer::YLineEditTextChange_hndl(const QString& text)
{
	if ((singlepointRow != -1)) {
...
void arnEnCompositeArrayViewer::enterEvent(QEvent */*e*/)
{
	if (pClip) {
		if (pClip->supportsSelection()) {
...
// void arnEnCompositeArrayViewer::resizeEvent ( QResizeEvent * e )
// {
// 
// }

void arnEnCompositeArrayViewer::paintEvent ( QPaintEvent * event )
{
...
void arnEnCompositeArrayViewer::defCChange_Hndl(void)
{
	QColor c = QColorDialog::getColor (QColor("white"), this,"dataColorDialog");
	if (c.isValid()) {emit defColorChanged(c);}
}

void arnEnCompositeArrayViewer::CChange_Hndl(void)
{
/*	if (!pArray) return;
	QColor c = QColorDialog::getColor (QColor("white"), this,"dataColorDialog");
	if (c.isValid()) {
